{
  var PR = require("./pnodes")

  // заглушка на случай отсутствия options.raw
...
lineTerminatorSequence "конец строки"
  = "\n"
  / "\r\n"
  / "\r"
  / "\u2028"
...
codeBlock
  = s:statement TERMINATOR?  { return s; }
//  = s:statement  lineTerminator ss:(statement)* TERMINATOR? { return [s,ss]} 
...
//assignment
//  = i:identifier _ '=' _ s:(sourceCharacter*) { return ['assignment',i,s]; }
...
assignmentOp
  = left:Assignable _ "=" !"=" right:
    ( TERMINDENT e:primaryExpression DEDENT { return e; }
    / TERMINATOR? _ e:primaryExpression { return e; }
    ) {
      return rp(new PR.AssignOp(left, right));
    }
...
return 
  = 'вернуться' / 'возврат' / 'обратно'
continue 
  = 'продолжить' / 'следующий' / 'дальше'
...
PRAVDA = $("правда" !identifierPart)
DA = $("да" !identifierPart)
ISTINA = $("истина" !identifierPart)
VERNO = $("верно" !identifierPart)
EST = $("есть" !identifierPart)
LOZH = $("ложь" !identifierPart)
NET = $("нет" !identifierPart)
FIGNYA = $("фигня" !identifierPart)
...
/* Простые типы данных */ 
bool
  = ( PRAVDA / DA / ISTINA / VERNO / EST) { return rp(new PR.Bool(true)); }
  / (LOZH / NET / FIGNYA) { return rp(new PR.Bool(false)); }

Numbers
  = "0b" bs:$(bit+) { return rp(new PR.Int(parseInt(bs, 2))); }
  / "0o" os:$(octalDigit+) { return rp(new PR.Int(parseInt(os, 8))); }
  / "0x" hs:$(hexDigit+) { return rp(new PR.Int(parseInt(hs, 16))); }
  / base:decimal e:[eE] sign:[+-]? exponent:decimal {
      return rp(new PR.Float(parseFloat('' + base.data + e + (sign ? sign : '') + exponent.data, 10)));
    }
  / decimal
...
decimal
  ...
  = integral:integer fractional:$("." decimalDigit+)? {
      return fractional
        ? rp(new PR.Float(parseFloat(integral + fractional, 10)))
        : rp(new PR.Int(+integral));
    }
...
string
  = "\"\"\"" d:(stringData / "'" / $("\"" "\""? !"\""))+ "\"\"\"" {
      return rp(new PR.String(stripLeadingWhitespace(d.join(''))));
    }
  / "'''" d:(stringData / "\"" / "#" / $("'" "'"? !"'"))+ "'''" {
      return rp(new PR.String(stripLeadingWhitespace(d.join(''))));
    }
  / "\"" d:(stringData / "'")* "\"" { return rp(new PR.String(d.join(''))); }
  / "'" d:(stringData / "\"" / "#")* "'" { return rp(new PR.String(d.join(''))); }
